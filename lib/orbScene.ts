...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  const chromaticShader = {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        gl_FragColor = vec4(cr.r * 0.65, cg.g * 0.9, cb.b * 1.1, 1.0) * flicker;
        // Push towards an electric-blue holographic tone
        gl_FragColor.rgb = mix(gl_FragColor.rgb, gl_FragColor.rgb * vec3(0.55, 0.8, 1.25), 0.3);
      }
    `,
  };
  ...
  // ——— COLORS ———
  const C_BRIGHT = 0x36b8ff;
  const C_MID = 0x147cce;
  const C_DIM = 0x0a3e7a;
  const C_FAINT = 0x061f45;
  const C_HOT = 0x8bdcff;
  ...
  function makeTextSprite(text: string, size = 0.08) {
    ...
    ctx.fillStyle = `rgba(${(40 + Math.random() * 35) | 0}, ${(155 + Math.random() * 65) | 0}, 255, ${alpha})`;
  ...
  g.addColorStop(0, "rgba(54,184,255,1)");
  g.addColorStop(0.2, "rgba(20,124,206,0.6)");
  g.addColorStop(0.5, "rgba(10,62,122,0.15)");
  g.addColorStop(1, "rgba(6,31,69,0)");
  dCtx.fillStyle = g;
